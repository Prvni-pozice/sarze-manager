import { NextResponse } from "next/server";
import { db } from "@/lib/db";
import { inventory, batches, productions, items } from "@/db/schema";
import { sql, eq, and, gt } from "drizzle-orm";

export async function GET() {
  const [
    itemCount,
    batchCount,
    openProductions,
    lowStock,
    recentMovements,
  ] = await Promise.all([
    // Celkový počet položek
    db.select({ count: sql<number>`count(*)` }).from(items),

    // Aktivní šarže (s nenulovým stavem)
    db.select({ count: sql<number>`count(*)` })
      .from(batches),

    // Otevřené výroby (draft nebo in_progress)
    db.select({ count: sql<number>`count(*)` })
      .from(productions)
      .where(sql`status IN ('draft', 'in_progress')`),

    // Položky pod minimálním stavem
    db.execute(sql`
      SELECT i.id, i.name, i.min_stock,
             COALESCE(SUM(inv.amount), 0) AS stock
      FROM items i
      LEFT JOIN inventory inv ON inv.item_id = i.id
      GROUP BY i.id, i.name, i.min_stock
      HAVING COALESCE(SUM(inv.amount), 0) < COALESCE(i.min_stock, 0)
      ORDER BY stock ASC
      LIMIT 5
    `),

    // Posledních 10 pohybů
    db.execute(sql`
      SELECT inv.id, inv.move_type, inv.amount, inv.moved_at,
             i.name AS item_name,
             b.batch_code
      FROM inventory inv
      JOIN items i ON i.id = inv.item_id
      JOIN batches b ON b.id = inv.batch_id
      ORDER BY inv.moved_at DESC
      LIMIT 10
    `),
  ]);

  return NextResponse.json({
    itemCount: Number(itemCount[0].count),
    batchCount: Number(batchCount[0].count),
    openProductions: Number(openProductions[0].count),
    lowStock,
    recentMovements,
  });
}
